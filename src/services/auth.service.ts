import { ICreateUserQuery, IUser } from "../interfaces";
import { ApiError, BAD_REQUEST, CONFLICT, FORBIDDEN, generateCode, GONE, INTERNAL_SERVER_ERROR, MAGIC_NUMBERS, NOT_FOUND, UNAUTHORIZED } from "../utils";
...
class AuthService {
    ...
    async register(
       { data : { name, email, password, phone }, files } : { data: ICreateUserQuery, files: any } 
    ) {
        try {
            ...
            const userCredentials: ICreateUserQuery = { name, email, password: hashedPassword, phone };
    ...
    private async createNewUser(data: ICreateUserQuery) {
        return await userService.createNewUser(data) as IUser;
    }
    ...
    async verifyEmail(token: string) {
        try {
            ...
            await Promise.all([
                userService.updateOne({ userId: user._id, role: user.role, data: { isVerified: true } }),
    ...
    async login({ email, password }: { email: string, password: string }) {
        try {
            ...
            if (!user || !user.password) {
                throw new ApiError('البريد الإلكتروني أو كلمة المرور غير صحيحة', BAD_REQUEST);
            }
    ...
    async resetPassword({ password, otp }: { password: string, otp: string }) {
        try {
            ...
            await Promise.all([
                ...
                userService.updateOne({ userId, role: userExist.role, data: { password: hashedPassword } }),
